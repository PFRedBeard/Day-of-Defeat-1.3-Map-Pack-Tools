...
sub modifyres {
    ...
    for my $map (@maps) {
        ...
        $overviewtxt =~ s/\\/\//gxms;
        my $overviewbmp = getoverview( $overviewtxt, "$mapbase.bmp" );
        $overviewbmp =~ s/\\/\//gxms;
...
https://github.com/PFRedBeard/Day-of-Defeat-1.3-Map-Pack-Tools
END
...
sub getoverview {
    my ( $overviewtxt, $mapbase ) = @_;

    # Set path for overview.bmp file and a full test path.
    my $overviewbmp = catfile( $overview_dir, $mapbase );
    my $testbmp     = catfile( $main_dir,     $overview_dir, $mapbase );

    # If overview exists assume that is correct and return.
    if ( -f $testbmp ) { return $overviewbmp; }

    # If text file does not exist return $overviewbmp
    my $testtxt = catfile( $main_dir, $overviewtxt );
    if ( !-f $testtxt ) { return $overviewbmp; }

    # Otherwise lets open up the txt file and pull the overview
    open my $file, '<', $testtxt or croak $!;
    my @temp = grep { /IMAGE/xms } <$file>;
    close $file or croak $!;

    # Place first line into $bmp
    my $bmp = shift @temp;

    # Remove IMAGE, all whitespace, and quotes
    $bmp =~ s/IMAGE//xms;
    $bmp =~ s/\A\s+|\s+\z//gxms;
    $bmp =~ s/["]//gxms;
...
// Pucker Factor Map Pack Scripts can be found here:
// https://github.com/PFRedBeard/Day-of-Defeat-1.3-Map-Pack-Tools
END
